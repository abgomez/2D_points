...
def health_function(_type, _smell , _cm, rows):
...
def calculateHealth(codeAnalysis_path):
   if os.path.exists(codeAnalysis_path + "metrics.csv"): 
       with open(codeAnalysis_path + 'metrics.csv', newline='') as csvfile:
            #Using Dict Reader 
          ...
          for row in reader:
             h["loc"] = h["loc"] + health_function(row[_headers[0]].lower(), _headers[1], row[_headers[1]],rows)
             h["ctcr"] = h["ctcr"] + health_function(row[_headers[0]].lower(), _headers[2], row[_headers[2]],rows)
             h["godc"] = h["godc"] + health_function(row[_headers[0]].lower(), _headers[3], row[_headers[3]],rows)
             h["inai"] = h["inai"] + health_function(row[_headers[0]].lower(), _headers[4], row[_headers[4]],rows)
             h["nop"] = h["nop"] + health_function(row[_headers[0]].lower(), _headers[5], row[_headers[5]],rows)
